...
// require no-category
if( _hui('no_categoty') ){
	require_once get_stylesheet_directory() . '/inc/no-category.php';
}

// require simple-url
// require_once get_stylesheet_directory() . '/inc/simple-url.php';

// require functions for admin
if( is_admin() ){
    require_once get_stylesheet_directory() . '/inc/fn-admin.php';
}
...
if (function_exists('add_theme_support')) {
	...
	set_post_thumbnail_size(220, 150, false );
}
...
function _get_post_thumbnail($size = 'thumbnail', $class = 'thumb') {
	...
	if (has_post_thumbnail()) {
		...
		if( !$issrc ){
        	...
        	$html = sprintf('<img src="%s" class="%s" alt="%s">', get_stylesheet_directory_uri() . '/img/thumbnail.png', $class, $post->post_title._get_delimiter().get_bloginfo('name'));
        }
